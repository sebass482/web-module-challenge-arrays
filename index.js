...
console.log(flavor31(originalFlavors))
let coolArr = []
console.log(flavor31(coolArr))
...
function addFlavor(flavorToBeAdded,arrayToBeAddedTo){
    arrayToBeAddedTo.unshift(flavorToBeAdded);
    return(arrayToBeAddedTo);
}
console.log(addFlavor("Pistachio",originalFlavors));
...
function removeLastFlavor(arrayToBeRemovedFrom){
    arrayToBeRemovedFrom.pop();
...
function getFlavorByIndex(arrayToGetFlavorFrom,indexNumber){
    return (arrayToGetFlavorFrom[indexNumber]);
}
...
function removeFlavorByName(arrayToRemoveFrom,flavorToRemove){
    for(let i=0;i<arrayToRemoveFrom.length;i++){
        if (flavorToRemove === arrayToRemoveFrom[i]){
